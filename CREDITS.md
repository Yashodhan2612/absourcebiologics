...
`public/assets/webgl/culture-field-poster.avif` is generated by
`scripts/generate-posters.mjs`, which rasterises the same chain layout the
hero shader uses. Original output; no third-party licence applies.

**Shader code is original.** The ellipsoid-impostor lighting in
`webgl/shaders/streptococcus.ts` and the 3D gradient noise in
`webgl/shaders/milkToCurd.ts` were both written for this repo, specifically so
that no third-party shader licence — Ashima's simplex noise being the usual one
— needs tracking here.
